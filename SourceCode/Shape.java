...
import static SourceCode.Board.labelScore;
...
public class Shape {
    ...
    private boolean collision = false, moveX;
    ...
    public void update() {
        ...
        if (collision) {
            ...
            checkLine();
            board.setNextShape();
            board.setNextShapeDemo(); //wanring line (updated at 12AM13 21/10)
        }
    ...
    public void render(Graphics g) {
        for (int row = 0; row < coords.length; row++) {
            for (int col = 0; col < coords[row].length; col++) {
                ...
                // x , y = 9 if you want to need next step of random math
            }
        }
    }

    // I created this method at 21/10 11AM
    public void renderDemo(Graphics g) {
        for (int row = 0; row < coords.length; row++) {
            for (int col = 0; col < coords[row].length; col++) {
                if (coords[row][col] != 0) {
                    g.drawImage(block, col * board.getBlockSize() + 13 * board.getBlockSize(), row * board.getBlockSize() + 11 * board.getBlockSize(), null);
                    // do not change value at this method
                }
            }
        }
    }
...
//"C:\\Users\\User\\Downloads\\icons8-square-30.png"